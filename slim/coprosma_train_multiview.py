...
tf.app.flags.DEFINE_string(
    'dataset_name', None, 'The name of dataset.')
...
def _get_init_fn(checkpoint_path, expand, restore_logits):
...
def one_train_cycle(checkpoint_path, expand_logits, restore_logits=True):
  ...
  with tf.Graph().as_default():
    ...
    # dataset = dataset_factory.get_split(FLAGS.dataset_split_name, FLAGS.dataset_dir)
    from datasets import coprosma
    dataset = coprosma.get_split(FLAGS.dataset_split_name, FLAGS.dataset_dir)
    print('coprosma classes', dataset.num_classes)
    ...
    def network_fn(main, side):
      arg_scope = nets_factory.inception.inception_resnet_v2_multiview_arg_scope(weight_decay=FLAGS.weight_decay)
      with slim.arg_scope(arg_scope):
        return func(main, side, num_classes=(dataset.num_classes - FLAGS.labels_offset), is_training=True)
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      image_main, image_side = naturewatch_preprocessing.preprocess_for_train_multi(image, train_image_size, train_image_size)
      ...
      batch_queue = slim.prefetch_queue.prefetch_queue(
          [images_main, images_side, labels], capacity=2 * deploy_config.num_clones)
    ...
    def clone_fn(batch_queue):
      ...
      images_main, images_side, labels = batch_queue.dequeue()
      logits, end_points = network_fn(images_main, images_side)
    ...
    slim.learning.train(
        ...
        init_fn=_get_init_fn(checkpoint_path, expand_logits, restore_logits),
...
def default_schedule(checkpoint_path, **kwargs):
  one_train_cycle(checkpoint_path, False, True)
...
def main(_):
  ...
  default_schedule(checkpoint_path)
