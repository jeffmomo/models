...
class SpeciesDataset(object):
    ...
    def new_tensors_to_item(self, keys_to_tensors):
      ...
      if self.translate:
        out = tf.gather(self.dataset.get_translation_map(),
                        self.old_tensors_to_item(keys_to_tensors))
      else:
        out = self.old_tensors_to_item(keys_to_tensors)
  ...
  def __init__(self, level=None):
    ...
    self._NUM_CLASSES = 19027 if level is None else len(def_lst[level])
  ...
  def get_split(self, split_name, dataset_dir, file_pattern=None, reader=None):
    ...
    items_to_handlers = {
      ...
      'label': SpeciesDataset.ModdedTensor(self, 'image/class/label', translate=False if self.level is None else True),
