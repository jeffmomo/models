...
def preprocess_for_train_multi(image, height, width, bbox=None,
  ...
  with tf.name_scope(scope, 'distort_image', [image, height, width, bbox]):
    ...
    central_bb = tf.constant([0.3, 0.3, 0.7, 0.7],
                         dtype=tf.float32,
                         shape=[1, 1, 4])

    distorted_image_central, bbx = distorted_bounding_box_crop(image, central_bb, 0.5, (0.8, 1.2), (0.01, 0.15)) #tf.image.resize_image_with_crop_or_pad(image, 299, 299) #
    ...
    tf.image_summary('images_with_distorted_bounding_box_central', image_with_distorted_box_central)
    ...
    print(distorted_image, distorted_image_central)
    distorted_image = apply_with_random_selector(
        distorted_image,
        lambda x, method: tf.image.resize_images(x, [height, width], method=method),
        num_cases=num_resize_cases)
    distorted_image_central = apply_with_random_selector(
        distorted_image_central,
        lambda x, method: tf.image.resize_images(x, [height, width], method=method),
        num_cases=num_resize_cases)
...
def preprocess_for_eval_multi(image, height, width,
  ...
  with tf.name_scope(scope, 'eval_image', [image, height, width]):
    ...
    if height and width:
      # Resize the image to the specified height and width.
      image_main = tf.expand_dims(image, 0)
      image_main = tf.image.resize_bilinear(image_main, [height, width],
                                            align_corners=False)
      image_main = tf.squeeze(image_main, [0])

      image_central = tf.expand_dims(image, 0)
      image_central = tf.image.resize_bilinear(image_central, [height, width],
                                            align_corners=False)
